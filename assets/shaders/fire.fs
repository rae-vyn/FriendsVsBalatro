...
#ifndef FNC_MOD289
#define FNC_MOD289
number mod289(const in number x) { return x - floor(x * (1. / 289.)) * 289.; }
...
#ifndef FNC_PERMUTE
#define FNC_PERMUTE
number permute(const in number v) { return mod289(((v * 34.0) + 1.0) * v); }
...
number taylorInvSqrt(in number r) { return 1.79284291400159 - 0.85373472095314 * r; }
...
number snoise(in vec4 v) {
    ...
    vec4 x4 = x0 + C.wwww;
    i = mod289(i);
    number j0 = permute( permute( permute( permute(i.w) + i.z) + i.y) + i.x);
...
vec2 snoise2( vec2 x ){
    number s  = snoise(vec2( x ));
    number s1 = snoise(vec2( x.y - 19.1, x.x + 47.2 ));
    return vec2( s , s1 );
}
vec3 snoise3( vec3 x ){
    number s  = snoise(vec3( x ));
    number s1 = snoise(vec3( x.y - 19.1 , x.z + 33.4 , x.x + 47.2 ));
    number s2 = snoise(vec3( x.z + 74.2 , x.x - 124.5 , x.y + 99.4 ));
    return vec3( s , s1 , s2 );
}
vec3 snoise3( vec4 x ){
    number s  = snoise(vec4( x ));
    number s1 = snoise(vec4( x.y - 19.1 , x.z + 33.4 , x.x + 47.2, x.w ));
    number s2 = snoise(vec4( x.z + 74.2 , x.x - 124.5 , x.y + 99.4, x.w ));
    return vec3( s , s1 , s2 );
}
#endif
...
number random(in vec4 pos) {
#ifdef RANDOM_SINLESS
    pos = fract(pos * RANDOM_SCALE);
    pos += dot(pos, pos.wzxy + 33.33);
    return fract((pos.x + pos.y) * (pos.z + pos.w));
#else
    number dot_product = dot(pos, vec4(12.9898,78.233,45.164,94.673));
    return fract(sin(dot_product) * 43758.5453);
#endif
}
vec2 random2(number p) {
...
vec2 srandom2(in vec2 p, const in number tileLength) {
    p = mod(p, vec2(tileLength));
    return srandom2(p);
}
vec3 srandom3(in vec3 p, const in number tileLength) {
...
number cubic(const in number v, in number slope0, in number slope1) {
    number a = slope0 + slope1 - 2.;
    number b = -2. * slope0 - slope1 + 3.;
    number c = slope0;
    number v2 = v * v;
    number v3 = v * v2;
    return a * v3 + b * v2 + c * v;
}
vec2 cubic(const in vec2 v, in number slope0, in number slope1) {
    number a = slope0 + slope1 - 2.;
    number b = -2. * slope0 - slope1 + 3.;
    number c = slope0;
    vec2 v2 = v * v;
...
vec3 cubic(const in vec3 v, in number slope0, in number slope1) {
    number a = slope0 + slope1 - 2.;
    number b = -2. * slope0 - slope1 + 3.;
    number c = slope0;
    vec3 v2 = v * v;
...
vec4 cubic(const in vec4 v, in number slope0, in number slope1) {
    number a = slope0 + slope1 - 2.;
    number b = -2. * slope0 - slope1 + 3.;
    number c = slope0;
    vec4 v2 = v * v;
...
number gnoise(vec2 st) {
    ...
    vec2 f = fract(st);
    number a = random(i);
    number b = random(i + vec2(1.0, 0.0));
    number c = random(i + vec2(0.0, 1.0));
    number d = random(i + vec2(1.0, 1.0));
    vec2 u = cubic(f);
    return mix( a, b, u.x) +
                (c - a)* u.y * (1.0 - u.x) +
                (d - b) * u.x * u.y;
}
number gnoise(vec3 p) {
    vec3 i = floor(p);
    vec3 f = fract(p);
    vec3 u = quintic(f);
...
FBM_NOISE_TYPE fbm(in vec2 st) {
    ...
    number amplitude = FBM_AMPLITUDE_INITIAL;
...
FBM_NOISE_TYPE fbm(vec3 p, number tileLength) {
    const number persistence = 0.5;
    const number lacunarity = 2.0;
    number amplitude = 0.5;
    FBM_NOISE_TYPE total = FBM_NOISE_TYPE(0.0);
    number normalization = 0.0;
    for (int i = 0; i < FBM_OCTAVES; ++i) {
        number noiseValue = FBM_NOISE3_TILABLE_FNC(p, tileLength * lacunarity * 0.5) * 0.5 + 0.5;
...
vec4 fire_col(vec2 _st, number _time) {
    number time = _time * 0.9;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    tex.rgb = 1.4 - tex.rgb;
    vec4 fire_color = fire_col(uv, -1.0 * (fire.g + fire.r)/2);
...
vec4 dissolve_mask(vec4 tex, vec2 texture_coords, vec2 uv)
{
    ...
    number adjusted_dissolve = (dissolve*dissolve*(3.-2.*dissolve))*1.02 - 0.01; //Adjusting 0.0-1.0 to fall to -0.1 - 1.1 scale so the mask does not pause at extreme values
...
extern PRECISION number hovering;
extern PRECISION number screen_scale;
